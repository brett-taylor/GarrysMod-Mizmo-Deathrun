...
function PS:LoadItems()	
	local _, dirs = file.Find('deathrun/gamemode/Pointshop/ShopItems/*', 'LUA')
	
	for _, category in pairs(dirs) do
		local f, _ = file.Find('deathrun/gamemode/Pointshop/ShopItems/' .. category .. '/__category.lua', 'LUA')
		...
		if #f > 0 then
			...
			if SERVER then AddCSLuaFile('deathrun/gamemode/Pointshop/ShopItems/' .. category .. '/__category.lua') end
			include('deathrun/gamemode/Pointshop/ShopItems/' .. category .. '/__category.lua')
			
			if not PS.Categories[category] then
				PS.Categories[category] = CATEGORY
			end
			
			local files, _ = file.Find('deathrun/gamemode/Pointshop/ShopItems/' .. category .. '/*.lua', 'LUA')
			
			for _, name in pairs(files) do
				if name ~= '__category.lua' then
					if SERVER then AddCSLuaFile('deathrun/gamemode/Pointshop/ShopItems/' .. category .. '/' .. name) end
					...
					include('deathrun/gamemode/Pointshop/ShopItems/' .. category .. '/' .. name)
