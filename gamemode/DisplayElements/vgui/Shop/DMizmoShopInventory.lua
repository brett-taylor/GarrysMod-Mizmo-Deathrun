...
function PANEL:Init()
	...
	for category_id, CATEGORY in pairs(PS.Categories) do
		if (CATEGORY.Name ~= "Weapons") then
			self.categoryButtons[#self.categoryButtons + 1] = self.CategoryGrid:Add("DMizmoButton");
			self.categoryButtons[#self.categoryButtons]:SetText(CATEGORY.Name);
			self.categoryButtons[#self.categoryButtons].OnClicked = function()
...
function PANEL:LoadItemTab(catName)
	...
	for _, ITEM in pairs(PS.Items) do
		if ITEM.Category == catName then
			if (LocalPlayer():PS_HasItem(ITEM.ID) == true) then
	...
	if (self.categoryButtons ~= nil) then
		for i = 1, #self.categoryButtons do
			if (self.categoryButtons[i] ~= nil) then
				self.categoryButtons[i]:SetSize(self.categoryButtons[i]:GetParent():GetParent():GetParent():GetParent():GetWide() - 10, 50);
			end
		end
	end
...
vgui.Register("DMizmoShopInventory", PANEL);